    ...
    while hacker_addresses_queue:
        ...
        if response.status_code == 200 and 'txs' in data:
            if data.get('n_tx', 0) >= 3000:  # 수취인 주소가 3000개 이상의 트랜잭션을 가지고 있는 경우 종료
                ...
                break
            sender_info = fetch_wallet_info(current_hacker_address)
            ...
            for tx in data['txs']:
                for output in tx['out']:
                    if 'addr' in output:
                        ...
                        receiver_info = fetch_wallet_info(receiving_wallet)
                        ...
                        transaction_data = {
                            ...
                            'sender_n_tx': sender_info['n_tx'],
                            'sender_total_received': sender_info['total_received'],
                            'sender_total_sent': sender_info['total_sent'],
                            'sender_final_balance': sender_info['final_balance'],
                            'receiver_n_tx': receiver_info['n_tx'],
                            'receiver_total_received': receiver_info['total_received'],
                            'receiver_total_sent': receiver_info['total_sent'],
                            'receiver_final_balance': receiver_info['final_balance']
                        }
